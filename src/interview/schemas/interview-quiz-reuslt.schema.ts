...
export class ResumeQuizResult {
  ...
  @Prop()
  matchLevel?: string; // 匹配等级（优秀/良好/中等/较差）
  ...
  radarData?: RadarDimension[]; // 雷达图数据
  @Prop({ type: [String], default: [] })
  strengths?: string[]; // 优势
  @Prop({ type: [String], default: [] })
  ...
  @Prop()
  aiModel?: string; // 使用的AI模型（如
  @Prop()
  promptVersion?: string; // 使用的提示版本(用于A/B测试)
}
