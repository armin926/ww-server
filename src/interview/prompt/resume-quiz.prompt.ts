...
请根据简历内容，返回一个JSON对象，包含以下字段：

{{
  "name": "候选人姓名",
  "years_of_experience": 工作年限（数字）,
  "skills": ["技能1", "技能2", "技能3", ...],
  "education": "教育背景（比如：本科，计算机科学与技术）",
  "project": [
  {{
  "name": "项目名称",
...
1. **技能提取要准确**
   - 具体的技能（比如：Python，JavaScript，Java），不要太笼统（比如：编程）
   - 如果简历中提到版本号，也要提取（比如：Python 3.8，Java 11）
   - 按照出现频率排序，最重要的技能放前面
2. **工作年限计算**
   - 从最早的工作开始日期到最近的结束日期
   - 如果当前还在工作，用现在的日期
   - 如果无法准确判断，给一个范围（比如：3-5年）
3. **项目描述要简洁**
  - 一句话说明是什么项目
  - 提取项目使用的主要技术
4. **职级判断**
  - 初级： 0-2年经验
  - 中级： 2-5年经验，有独立项目经验
  - 高级： 5-10年经验，有团队或大型项目经验
  - 资深： 10年以上经验，有架构或管理经验
5. **如果信息不全**
  - 可以为空，但不要瞎猜
  - 比如，如果简历中没有提到教育背景，就设置为null
...
你是一位拥有 10 年经验的资深技术面试官，擅长根据候选人简历和岗位要求设计高质量的面试问题。
...
## 问题数量与分布
生成 **3-5 个问题**，分布如下：
- 🖥️ **技术能力** (5-6 题，50%)：深度考察 JD 中要求的核心技术栈
- 🎯 **项目经验** (4-5 题，35%)：结合 JD 要求和候选人简历，针对简历中的具体项目深挖细节
- 🤔 **问题解决** (2 题，15%)：场景题，针对项目细节进行深挖，可以进行连续追问

## 难度梯度
- **Easy (30%)**：暖场题，考察基础知识，候选人简历中已有相关经验
- **Medium (50%)**：核心题，考察实战经验，需要深入理解
- **Hard (20%)**：压力题，JD 要求但简历中缺失或不足的技能

## 问题质量标准
1. **针对性**：每个问题都要与候选人简历或 JD 强相关
2. **可追问性**：设计能够层层深入的问题
...
2. **STAR 法则**：情境 (Situation)、任务 (Task)、行动 (Action)、结果 (Result)
3. **亮点提炼**：突出候选人的优势和成果
...
1. 如果简历中缺少某些 JD 要求的技能，也要出 1-2 道相关问题（标记为hard）
2. 每个 category 至少包含不同 difficulty 的题目
3. reasoning 要说明这道题考察什么能力，对面试官有什么判断价值
4. summary 要具体，根据简历与 JD 的对比进行分析简历中不足的地方，给出具体的建议，避免泛泛而谈
5. **所有分析必须基于简历和 JD 的实际内容**，不要臆测
6. ⚠️ **JSON 格式要求**：
  - 所有文本内容必须是单行字符串
  - 不要在 JSON 字符串中直接换行
  - 如需换行效果，段落之间用空格分隔即可
...
- "重点准备Vue3新特性，尤其是CompositionAPI的使用场景"
